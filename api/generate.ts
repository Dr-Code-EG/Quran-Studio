...
export default async (req: Request, res: Response) => {
  if (req.method === 'POST') {
    upload.fields([{ name: 'backgrounds', maxCount: 10 }, { name: 'watermark', maxCount: 1 }, { name: 'customFont', maxCount: 1 }])(req as any, res as any, async err => {
      if (err) {
        return res.status(400).json({ error: 'File upload failed: ' + err.message });
      }

      if (!db) {
        return res.status(500).json({ error: 'Database not initialized. SQLite might not be supported in this environment.' });
      }
      ...
      try {
        ...
        if ((req as any).files['backgrounds']) {
          (req as any).files['backgrounds'].forEach((file: any, index: number) => {
        ...
        if ((req as any).files['customFont']) {
          config.customFontUrl = `/uploads/${(req as any).files['customFont'][0].filename}`;
        }
